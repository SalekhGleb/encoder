...
class CryptoApp(ttk.Window):
    ...
    def create_widgets(self):
        ...
        settings_menu.add_command(label="Light Theme", command=lambda: self.change_theme('light'))
        settings_menu.add_command(label="Dark Theme", command=lambda: self.change_theme('dark'))
        settings_menu.add_command(label="About", command=lambda: messagebox.showinfo("About", "Encoder v1.0\nApplication for text encryption and decryption"))
    ...
    def create_encrypt_tab(self):
        ...
        ttk.Label(frame, text="Text to Encrypt:", font=('Segoe UI', 10)).pack(anchor='w', padx=10, pady=(10, 5))
        self.encrypt_input = tk.Text(frame, height=10, font=('Segoe UI', 10), wrap=tk.WORD)
        self.encrypt_input.pack(fill=tk.X, padx=10, pady=10)
        self.encrypt_input.insert(tk.END, "Enter text to encrypt...")
        self.encrypt_input.bind("<FocusIn>", lambda e: self.clear_placeholder(self.encrypt_input, "Enter text to encrypt..."))
        self.encrypt_input.bind("<FocusOut>", lambda e: self.restore_placeholder(self.encrypt_input, "Enter text to encrypt..."))
        ...
        ttk.Label(key_frame, text="Key:", font=('Segoe UI', 10)).pack(side=tk.LEFT, padx=(0, 5))
        self.key_entry = ttk.Entry(key_frame, width=50, font=('Segoe UI', 10))
        self.key_entry.pack(side=tk.LEFT, padx=5)
        self.key_entry.insert(0, "Enter or generate key...")
        self.key_entry.bind("<FocusIn>", lambda e: self.clear_placeholder(self.key_entry, "Enter or generate key..."))
        self.key_entry.bind("<FocusOut>", lambda e: self.restore_placeholder(self.key_entry, "Enter or generate key..."))
        self.generate_key_btn = ttk.Button(key_frame, text="Generate Key", command=self.generate_random_key, bootstyle="info")
        self.generate_key_btn.pack(side=tk.LEFT, padx=5)
        key_frame.pack(fill=tk.X, padx=10, pady=10)
        ttk.Button(frame, text="Encrypt", command=self.perform_encryption, bootstyle="success", width=20).pack(pady=15)
        ttk.Label(frame, text="Result:", font=('Segoe UI', 10)).pack(anchor='w', padx=10, pady=(10, 5))
        self.encrypt_output = tk.Text(frame, height=10, font=('Segoe UI', 10), wrap=tk.WORD)
    ...
    def create_decrypt_tab(self):
        frame = ttk.Frame(self.notebook, padding=10)
        ttk.Label(frame, text="Encrypted Text:", font=('Segoe UI', 10)).pack(anchor='w', padx=10, pady=(10, 5))
        self.decrypt_input = tk.Text(frame, height=10, font=('Segoe UI', 10), wrap=tk.WORD)
        self.decrypt_input.pack(fill=tk.X, padx=10, pady=10)
        self.decrypt_input.insert(tk.END, "Enter encrypted text...")
        self.decrypt_input.bind("<FocusIn>", lambda e: self.clear_placeholder(self.decrypt_input, "Enter encrypted text..."))
        self.decrypt_input.bind("<FocusOut>", lambda e: self.restore_placeholder(self.decrypt_input, "Enter encrypted text..."))
        ...
        ttk.Label(frame, text="Key:", font=('Segoe UI', 10)).pack(anchor='w', padx=10, pady=(10, 5))
        self.decrypt_key_entry = ttk.Entry(frame, width=60, font=('Segoe UI', 10))
        self.decrypt_key_entry.pack(fill=tk.X, padx=10, pady=10)
        self.decrypt_key_entry.insert(0, "Enter key...")
        self.decrypt_key_entry.bind("<FocusIn>", lambda e: self.clear_placeholder(self.decrypt_key_entry, "Enter key..."))
        self.decrypt_key_entry.bind("<FocusOut>", lambda e: self.restore_placeholder(self.decrypt_key_entry, "Enter key..."))
        ttk.Button(frame, text="Decrypt", command=self.perform_decryption, bootstyle="success", width=20).pack(pady=15)
        ttk.Label(frame, text="Result:", font=('Segoe UI', 10)).pack(anchor='w', padx=10, pady=(10, 5))
        self.decrypt_output = tk.Text(frame, height=10, font=('Segoe UI', 10), wrap=tk.WORD)
    ...
    def create_history_tab(self):
        ...
        self.history_tree = ttk.Treeview(frame, columns=columns, show='headings', selectmode='browse', bootstyle="info")
        self.history_tree.heading('datetime', text='Date & Time')
        self.history_tree.heading('operation', text='Operation')
        self.history_tree.heading('text', text='Original Text')
        ...
        scrollbar = ttk.Scrollbar(frame, orient=tk.VERTICAL, command=self.history_tree.yview, bootstyle="round")
        self.history_tree.configure(yscroll=scrollbar.set)
        scrollbar.pack(side=tk.RIGHT, fill=tk.Y)
        self.history_tree.pack(fill=tk.BOTH, expand=True, padx=10, pady=10)
        ttk.Button(frame, text="Clear History", command=self.clear_history, bootstyle="danger", width=20).pack(pady=15)
        self.update_history_view()
        return frame
    
    def add_history_record(self, operation, text, result):
        record = {'datetime': datetime.now().strftime("%Y-%m-%d %H:%M:%S"), 'operation': operation, 'text': text, 'result': result}
    ...
    def update_history_view(self):
        ...
        for record in reversed(self.history):
            self.history_tree.insert('', 'end', values=(record['datetime'], record['operation'], record['text'][:50] + '...' if len(record['text']) > 50 else record['text'], record['result'][:50] + '...' if len(record['result']) > 50 else record['result']))
    ...
    def perform_encryption(self):
        ...
        if not text or not key or text == "Enter text to encrypt..." or key == "Enter or generate key...":
        ...
        try:
            ...
            self.encrypt_output.configure(state='disabled')
            self.add_history_record('Encryption', text, encrypted)
    ...
    def perform_decryption(self):
        ...
        if not text or not key or text == "Enter encrypted text..." or key == "Enter key...":
            messagebox.showerror("Error", "Fill in all fields")
            return
        try:
            ...
            self.decrypt_output.configure(state='disabled')
            self.add_history_record('Decryption', text, decrypted)
        except Exception as e:
            messagebox.showerror("Error", "Incorrect key or data")
